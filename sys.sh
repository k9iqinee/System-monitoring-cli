...
display_system_info() {
    ...
    # Disk info for both / and /home
    local root_used=$(df -h / | awk 'NR==2 {print $3}')
    local root_total=$(df -h / | awk 'NR==2 {print $2}')
    local root_percent=$(df -h / | awk 'NR==2 {print $5}' | tr -d '%')

    local home_used=$(df -h /home | awk 'NR==2 {print $3}')
    local home_total=$(df -h /home | awk 'NR==2 {print $2}')
    local home_percent=$(df -h /home | awk 'NR==2 {print $5}' | tr -d '%')
    ...
    echo -e "${YELLOW} Root (/):  ${GREEN}${root_used}/${root_total} (${root_percent}%)"
    echo -e "${YELLOW} Home:      ${GREEN}${home_used}/${home_total} (${home_percent}%)"
